...
export class AccesoWebComponent {
  ...
  private LlenarDatos()
  {
    ...
    if(this.cmbUsuario?.select.length > 0)
    {
      ...
      Acc.forEach(f =>{
        ...
        let i = this.lstDatosPerfil.findIndex((w : any) => w.Usuario == u && w.Id == f.Id && w.Activo && w.Modulo == "FACT");
        f.Activo = false;
        f.IdAcceso = -1;
        f.Usuario = u;
        if(i != -1) 
  ...
  V_Activar(checked: any, element : any): void {
    ...
    let i : number = this.lstPerfil.data.findIndex(f => f.MenuPadre == element.MenuPadre && f.Activo && f.Modulo == "FACT");
